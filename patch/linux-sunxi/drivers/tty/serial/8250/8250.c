	...
	struct uart_8250_port *up =
        ...
        if ((iir & 0x7) == UART_IIR_BUSY) {
                if (serial_in(up, UART_USR)&1) {
                        serial_out(up, UART_HALT, UART_FORCE_CFG);
                        serial_out(up, UART_LCR, up->lcr);
                        serial_out(up, UART_HALT, UART_FORCE_CFG |UART_FORCE_UPDATE);
                        while(serial_in(up, UART_HALT)&UART_FORCE_UPDATE);
                        serial_out(up, UART_HALT, 0x00);
		          ...
		          struct ktermios *old)
{
	...
	* */
	...
	serial_out(up, UART_LCR, cval);
        ...
        * */
       if(serial_in(up, UART_HALT) & UART_FORCE_CFG){
               serial_out(up, UART_HALT, UART_FORCE_CFG | UART_FORCE_UPDATE);
               while(serial_in(up, UART_HALT) & UART_FORCE_UPDATE);
               serial_out(up, UART_HALT, 0x00);
       }
