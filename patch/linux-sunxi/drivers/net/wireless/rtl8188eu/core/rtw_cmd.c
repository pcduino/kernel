...
#define TRAFFIC_PROTECT_PERIOD_MS 4500
...
#if 0
	{
		...
		if( is_primary_adapter(padapter)){			
			...
			printk("Primary adapter ==> NumRxOkInPeriod:%d, NumTxOkInPeriod:%d \n",
				pmlmepriv->LinkDetectInfo.NumRxOkInPeriod,pmlmepriv->LinkDetectInfo.NumTxOkInPeriod );
			printk("Primary adapter ==> rx_bytes:%lld, tx_bytes:%lld \n",curRxOkCnt ,curTxOkCnt);
			
			
		}
		else{
			printk("Second adapter ==> NumRxOkInPeriod:%d, NumTxOkInPeriod:%d \n",
				pmlmepriv->LinkDetectInfo.NumRxOkInPeriod,pmlmepriv->LinkDetectInfo.NumTxOkInPeriod );
			printk("Second  adapter ==> rx_bytes:%lld, tx_bytes:%lld \n",curRxOkCnt ,curTxOkCnt);
		}
	}
#endif
