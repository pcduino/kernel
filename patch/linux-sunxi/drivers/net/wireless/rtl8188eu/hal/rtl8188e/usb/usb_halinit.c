...
{
	u8	value8;
	#if 0 //for open-space miracast test
...
//_RfPowerSave(Adapter);
	...
	if(pregistrypriv->wifi_spec)
		...
		rtw_write32(Adapter,REG_FAST_EDCA_CTRL ,0x03084444);
	//Nav limit , suggest by scott
	rtw_write8(Adapter, 0x652, 0x0);

	rtw_write8(Adapter,0x420,(rtw_read8(Adapter,0x420)&(~BIT(7))));
...
static void hw_var_set_opmode(PADAPTER Adapter, u8 variable, u8* val)
{
	...
	if(Adapter->iface_type == IFACE_PORT1)
	{
		...
		if((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_))
		{
			...
			rtw_write8(Adapter,REG_BCN_CTRL_1, 0x11);//disable atim wnd
	...
	)
{
	...
	switch(eVariable)
	{
		...
		case HAL_DEF_DBG_DM_FUNC:
			*(( u32*)pValue) =podmpriv->SupportAbility;
			break;
#if (RATE_ADAPTIVE_SUPPORT == 1)
		...
		case HW_DEF_ODM_DBG_FLAG:
			{
				...
				podmpriv->DebugComponents = DebugComponents;			
			}
			break;
