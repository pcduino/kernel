...
static int hdmi_mode= HDMI_MODE;

int get_hdmi_mode(void)
{
        printk("%s == %s\n", __FUNCTION__, (hdmi_mode == DVI_MODE)? "DVI":"HDMI");
        return hdmi_mode;
}

static int _get_hdmi_mode(u8* pdata)
{
    int i;
    int EDID_BLOCK_SIZE = 128;
        if(pdata[126] >= 0x01){
        for(i=4; i<pdata[EDID_BLOCK_SIZE+2]; ++i){
            if((pdata[EDID_BLOCK_SIZE+i] & 0xE0) == 0x60){ // find tag code - Vendor Specific Block
                if((pdata[EDID_BLOCK_SIZE+1+i] == 0x03) &&
                    (pdata[EDID_BLOCK_SIZE+2+i] == 0x0C) &&
                    (pdata[EDID_BLOCK_SIZE+3+i] == 0x00)){
                                        return HDMI_MODE;
                                }
                        }else{
                            // bit 7..5: Block Type Tag
                            // bit 4..0: Total number of bytes in this block following this byte
                                i += (pdata[EDID_BLOCK_SIZE+i] & 0x1F);
                        }
                }
                if(i >= pdata[EDID_BLOCK_SIZE+2]) { // Error, can not find the Vendor Specific Block
                        return DVI_MODE;
                }
        }
        return DVI_MODE;
...
#ifdef CONFIG_SUNXI_DVI_FIX
        hdmi_mode = _get_hdmi_mode(EDID_Buf);
        if ( hdmi_mode == DVI_MODE )
        {
                printk("set to DVI mode\n");
		writeb(0x80, HDMI_VIDEO_CTRL + 3);
        }
        else
        {
                printk("set to HDMI mode\n");
		writeb(0xc0, HDMI_VIDEO_CTRL + 3);
        }
#endif
